...
{
	ShowE('divMyPages'	, ( tabCode == 'MyPages' ) ) ;
...
function LoadMyPageOptions() {
	...
	var pageList = oEditor.FCK.Config['pageList'];

	if (pageList.length > 0) {
		var pageListArray = pageList.split('|');
		for (var page in pageListArray) {
			GetE('selectedPage').options[i] = new Option(pageListArray[page], pageListArray[page]);
			i++;
		}
		GetE('pageLabel').style.display = "inline";
		GetE('selectedPage').style.display = "inline";
		GetE('noPages').style.display = "none";

		// If we don't currently have a link, then set the link to the first item in the list
		if (!oLink) {
			SetMyPage(GetE('selectedPage').options[0].value);
		}
	}
	else {
		GetE('pageLabel').style.display = "none";
		GetE('selectedPage').style.display = "none";
		GetE('noPages').style.display = "inline";
	}
...
function SetMyPage(selectedPage) {
	...
	var myPageURL = oEditor.FCK.Config['siteURL'] + oEditor.FCK.Config['cwd'] + "/" + selectedPage + ".html";

	// Remove the protocol and get the remaining URL.
	var sUrl = myPageURL.replace( oRegex.UriProtocol, '' ) ;
	
	// Set the link type to be URL
	cmbLinkType.value = "url";
